...
export default defineTool({
  name: "gw_repo_state",
  description: "Groundwork provenance tool `gw_repo_state` via `groundwork provenance repo-state`.",
  input: Input,
  ...
  async handle(input) {
    return runGroundworkProvenance("repo-state", input as Record<string, unknown>);
  },
});
