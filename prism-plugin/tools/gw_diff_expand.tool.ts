...
export default defineTool({
  name: "gw_diff_expand",
  description: "Groundwork provenance tool `gw_diff_expand` via `groundwork provenance diff-expand`.",
  input: Input,
  ...
  async handle(input) {
    return runGroundworkProvenance("diff-expand", input as Record<string, unknown>);
  },
});
