...
export default defineTool({
  name: "gw_commit_expand",
  description: "Groundwork provenance tool `gw_commit_expand` via `groundwork provenance commit-expand`.",
  input: Input,
  ...
  async handle(input) {
    return runGroundworkProvenance("commit-expand", input as Record<string, unknown>);
  },
});
