...
export default defineTool({
  name: "gw_file_state",
  description: "Groundwork provenance tool `gw_file_state` via `groundwork provenance file-state`.",
  input: Input,
  ...
  async handle(input) {
    return runGroundworkProvenance("file-state", input as Record<string, unknown>);
  },
});
