...
export default defineTool({
  name: "gw_pr_expand",
  description: "Groundwork provenance tool `gw_pr_expand` via `groundwork provenance pr-expand`.",
  input: Input,
  ...
  async handle(input) {
    return runGroundworkProvenance("pr-expand", input as Record<string, unknown>);
  },
});
