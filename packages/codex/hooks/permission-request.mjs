...
var normalizePayload = (input) => {
  const target = { harness: "codex-cli", nativeEvent: "PermissionRequest" };
  const cwd = input?.cwd;
  switch ("permission.request") {
    ...
    case "session.start":
      return { target, cwd, session: nativeSession(input) ?? { id: "codex-cli" }, native: input };
    case "session.end":
      return { target, cwd, session: nativeSession(input) ?? { id: "codex-cli" }, reason: input?.reason, native: input };
    case "stop":
      return {
        target,
        cwd,
