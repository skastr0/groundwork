...
var tool_before_hook_default = defineHook({
  name: "tool-before",
...
var normalizePayload = (input) => {
  const target = { harness: "claude-code", nativeEvent: "PreToolUse" };
...
var payload = unwrapDecode(decodeNativeHookPayloadForEvent("tool.before", normalizePayload(await parseInput())), "native payload");
var rawResult = await toPromise(tool_before_hook_default.handle(payload));
var result = unwrapDecode(decodeHookResultForEvent("tool.before", rawResult ?? { decision: "continue" }), "result");
