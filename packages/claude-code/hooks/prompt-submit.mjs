...
var prompt_submit_hook_default = defineHook({
  name: "prompt-submit",
...
var normalizePayload = (input) => {
  const target = { harness: "claude-code", nativeEvent: "UserPromptSubmit" };
  const cwd = input?.cwd ?? input?.workspace?.cwd;
  switch ("prompt.submit") {
    case "tool.before":
...
var payload = unwrapDecode(decodeNativeHookPayloadForEvent("prompt.submit", normalizePayload(await parseInput())), "native payload");
var rawResult = await toPromise(prompt_submit_hook_default.handle(payload));
var result = unwrapDecode(decodeHookResultForEvent("prompt.submit", rawResult ?? { decision: "continue" }), "result");
