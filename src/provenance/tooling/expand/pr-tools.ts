...
async function resolveRemoteContext(options: {
  ...
  if (options.mode === "local") {
    return {
      status: "unsupported",
      attempted: false,
      requestedNumber: options.pr ?? null,
      resolvedNumber: null,
  ...
  if (prNumber === undefined) {
    ...
    if (!detectResult.success) {
      return {
        ...
        resolvedNumber: null,
        detectionMethod,
        confidence: detectResult.failure.confidence,
        code: detectResult.failure.code,
        message: detectResult.failure.message,
        retryable: detectResult.failure.retryable,
      };
    }
  ...
  const metadataCommand = `gh pr view ${prNumber} --json number,title,body,url,state,isDraft,author,baseRefName,headRefName,createdAt,updatedAt`;
  ...
  if (!metadataResult.success) {
    return {
      ...
      attempted: true,
      requestedNumber,
      resolvedNumber: prNumber,
      detectionMethod,
      confidence: metadataResult.failure.confidence,
      code: metadataResult.failure.code,
      message: metadataResult.failure.message,
      retryable: metadataResult.failure.retryable,
    };
  }
  ...
  if (!parsedMetadata.success) {
    return {
      ...
      resolvedNumber: prNumber,
      detectionMethod,
      confidence: parsedMetadata.failure.confidence,
      code: parsedMetadata.failure.code,
      message: parsedMetadata.failure.message,
      retryable: parsedMetadata.failure.retryable,
    };
  }
  ...
  return {
    ...
    attempted: true,
    requestedNumber,
    resolvedNumber: parsedMetadata.data.number,
    detectionMethod: `${detectionMethod} + ${GH_PR_FETCH_METHOD}`,
    confidence: inferRemoteConfidence(files, reviewContext),
    metadata: {
      number: parsedMetadata.data.number,
      title: parsedMetadata.data.title,
      url: parsedMetadata.data.url,
      state: parsedMetadata.data.state,
      isDraft: parsedMetadata.data.isDraft,
      author: parsedMetadata.data.author?.login ?? null,
      baseRefName: parsedMetadata.data.baseRefName,
      headRefName: parsedMetadata.data.headRefName,
      createdAt: parsedMetadata.data.createdAt,
      updatedAt: parsedMetadata.data.updatedAt,
    },
    description: buildBoundedText(parsedMetadata.data.body, options.maxBytes),
    files,
    reviewContext,
  };
}
