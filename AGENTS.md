...
Do not put the Groundwork readiness skill inside this runtime repo. The skill source lives in `/Users/guilhermecastro/Projects/ai-plugins/groundwork-readiness/skills/groundwork-readiness` and is installed into harness skill directories from there.
...
Before changing `groundwork.toml`, `.groundwork/*.toml`, `src/policy/**`, or harness integration files, load the `groundwork-readiness` skill and check whether an SDLC work item is required.
