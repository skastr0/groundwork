...
[2026-05-06]: Updated bundled Codex plugin hooks to call `$HOME/.local/bin/groundwork`, fixing the observed `hook exited with code 127` path for minimal hook environments without relying on `PATH` lookup.
...
[2026-05-06]: Consolidation review follow-up: retired the bundled hook `PATH` compatibility branch and made bundled hooks call `$HOME/.local/bin/groundwork` directly so stale global shims cannot take precedence over the canonical local install.
