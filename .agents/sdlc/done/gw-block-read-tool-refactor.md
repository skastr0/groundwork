...
[2026-05-06]: Addressed review findings by adding branch coverage for unsupported modes, invalid paths, range/window validation, success meta/summary/sources, and failure logging/envelope. Consolidated read and block-read path-normalization failures through one query-level serializer.
[2026-05-06]: Final `bun run verify` passed after review fixes with 27 files / 268 tests plus build/import/CLI/local-install checks.
